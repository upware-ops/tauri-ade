// forked from design-sync lib/story-imports.mjs — rule 2's per-import b.resolve made preview builds 200x slower here (61s vs 0.3s for one preview)
//
  ...
  const policyRedirect = {
    ...
    setup(b) {
      b.onResolve({ filter: /.*/ }, async (a) => {
        ...
        // FORK: rule 2 costs a full b.resolve round-trip per import, and the
        // bundled graph here is dominated by third-party files (lucide-react
        // alone is ~1k modules) — 0.3s of esbuild became 61s. These two
        // early-outs skip the round-trip for imports that provably cannot
        // land on a repo component: nothing under node_modules imports one,
        // and every DS import in this repo is spelled '@/...' (tsconfig
        // alias) or relative. Bare specifiers reach the dsShim rule above.
        if (a.importer.includes('/node_modules/')) return null;
        if (!/^(\.\.?\/|\/|@\/)/.test(a.path)) return null;
